...
> This workflow was completed in HPC commit `5f24bd4`. It reproduces the old frozen-transfer diagnostic, not the proposed new task. See `research/FOLLOWUP_PROPOSAL.md` for the next design; its E0–E2 pipeline remains to be implemented.
...
Monitor `gpu-JOB.log` and `utilization-JOB.log` in `$OUT`. GPU, CPU, job state, elapsed time, and recent progress are recorded every five seconds. Diagnose persistent low utilization or stalled progress promptly and switch to another prepared productive task when applicable. Do not manually cancel or release the allocation; the latest user instruction requires server-managed reclamation because manual release interrupts the session. This script performs extraction and inference only; CPU bootstrapping follows GPU work. It combines copied intervention rows for one `run.features` call, deduplicating shared crops, texts, and phrase embeddings with one encoder load. Each frozen scorer is loaded once for all interventions, and predictions are sliced back into per-intervention files. The original source rows remain unchanged. Serial image preparation remains an optimization opportunity. Do not claim utilization is fixed solely because monitoring is present. Completed feature caches survive a retry; interrupted in-progress extraction may need repeating.
