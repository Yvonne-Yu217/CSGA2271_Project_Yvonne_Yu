...
def features(rows):
    fingerprint_rows=[{k:r[k] for k in ['image_id','split','caption_index',MINUS_FIELD,'tplus','tcontrol','target_phrase','regions']} for r in rows]
    digest=hashlib.sha256(json.dumps(fingerprint_rows,sort_keys=True,separators=(',',':')).encode()).hexdigest()
    ...
    model=AutoModel.from_pretrained(MODEL,local_files_only=LOCAL_ONLY).to(device).eval()
    proc=AutoProcessor.from_pretrained(MODEL,local_files_only=LOCAL_ONLY,use_fast=False)
    texts=sorted({r[k] for r in rows for k in [MINUS_FIELD,'tplus','tcontrol']}); ti={t:i for i,t in enumerate(texts)}
    ...
    flat=[];ptr=[0]
    for r in rows:
        for reg in r['regions']:flat.append([vi[(r['image_id'],*reg['box'])],*[ti[r[k]] for k in [MINUS_FIELD,'tplus','tcontrol']]])
        ptr.append(len(flat))
    ...
    for r in rows:
        ...
        ss=[]
        for key in [MINUS_FIELD,'tplus','tcontrol']:
    ...
    (OUT/'feature_runtime.json').write_text(json.dumps(dict(device=device,gpu_name=gpu_name,seconds=time.time()-start,crops=len(crops),texts=len(texts),model=MODEL,torch=torch.__version__,transformers=__import__('transformers').__version__,python=platform.python_version(),fingerprint=digest),indent=2))
    return np.load(path)
...
class Scorer(nn.Module):
    def __init__(self,mode,feature_dim):
        super().__init__();self.mode=mode;self.net=nn.Sequential(nn.Linear(4*feature_dim,128),nn.ReLU(),nn.Linear(128,1),nn.Sigmoid())
...
def main():
    global DATA,OUT,DEVICE,BATCH_SIZE,LOCAL_ONLY,MINUS_FIELD,MODEL
    ...
    ap.add_argument('--available-images-only',action='store_true',help='Smoke-test only rows whose staged image currently exists')
    ap.add_argument('--max-images-per-split',type=int,default=0,help='Smoke-test cap per split; 0 keeps all images')
    args=ap.parse_args(); DATA=args.data_dir;OUT=args.output_dir;DEVICE=args.device;BATCH_SIZE=args.batch_size;LOCAL_ONLY=not args.allow_model_download;MINUS_FIELD=args.minus_field;MODEL=args.model
    ...
    start=time.time();rows=json.loads((DATA/'manifest.json').read_text())
    if args.available_images_only:
        rows=[r for r in rows if (DATA/'images'/(r['image_id']+'.jpg')).is_file()]
    rows=[r for r in rows if isinstance(r.get(MINUS_FIELD),str) and r[MINUS_FIELD].strip()]
    if args.max_images_per_split:
        keep=set()
        for split in ['train','val','test']:
            keep.update(dict.fromkeys(r['image_id'] for r in rows if r['split']==split).keys())
            split_ids=list(dict.fromkeys(r['image_id'] for r in rows if r['split']==split))[:args.max_images_per_split]
            keep.difference_update(set(dict.fromkeys(r['image_id'] for r in rows if r['split']==split))-set(split_ids))
        rows=[r for r in rows if r['image_id'] in keep]
    if not rows:ap.error('No experiment rows remain after smoke-test filters')
    (OUT/'manifest_snapshot.json').write_text(json.dumps(rows,indent=2))
    c=features(rows)
    train_device=DEVICE if DEVICE!='auto' else ('cuda' if torch.cuda.is_available() else ('mps' if torch.backends.mps.is_available() else 'cpu'))
    ptr=c['ptr'];ix=c['index'];v=torch.tensor(c['v'][ix[:,0]],device=train_device);ts=[torch.tensor(c['t'][ix[:,j]],device=train_device) for j in [1,2,3]]
    ...
    scores={};cos=np.stack([.5*(1-(v*t).sum(-1).cpu().numpy()) for t in ts],axis=1);scores['inverse_cosine']=cos
    scores['max_ngram_cosine']=c['ngram_scores']
    scores['constant']=np.full_like(cos,.5)
    scores['negative_text_length']=np.array([[-len(rows[i][k].split())/77 for k in [MINUS_FIELD,'tplus','tcontrol']] for i in range(len(rows)) for _ in rows[i]['regions']])
    scores['region_area']=np.array([[float((reg['box'][2]-reg['box'][0])*(reg['box'][3]-reg['box'][1]))]*3 for r in rows for reg in r['regions']])
    trainflat=torch.tensor(np.concatenate([np.arange(ptr[i],ptr[i+1]) for i in splits['train']]),device=train_device)
    targets=torch.tensor([ptr[i] for i in splits['train']],device=train_device); off=torch.tensor(np.concatenate([np.arange(ptr[i]+1,ptr[i+1]) for i in splits['train']]),device=train_device)
    labels=torch.zeros(len(v),device=train_device);labels[targets]=1
    ...
    for mode in ['rank','rank_local','rank_local_control','supervised_omission','text_only','image_only']:
        for seed in [0,1,2]:
            torch.manual_seed(seed);model=Scorer(mode,v.shape[1]).to(train_device);opt=torch.optim.AdamW(model.parameters(),lr=.001,weight_decay=.01);best=-1;beststate=None;bestepoch=0
            for epoch in range(1,61):
                model.train();qm,qp,qc=[model(v,t) for t in ts]
                if mode=='supervised_omission':
                    loss=nn.functional.binary_cross_entropy(qm[trainflat],labels[trainflat])+nn.functional.binary_cross_entropy(qp[trainflat],torch.zeros(len(trainflat),device=train_device))
                else:
                ...
                if epoch%5==0:
                    model.eval()
                    with torch.no_grad():ss=np.stack([model(v,t).cpu().numpy() for t in ts],axis=1)
                    metric=np.mean([x[4] for x in per_image(rows,ptr,ss,splits['val']).values()])
                    if metric>best:best=metric;beststate={k:x.detach().cpu().clone() for k,x in model.state_dict().items()};bestepoch=epoch
            model.load_state_dict(beststate);model.eval();name=f'{mode}_seed{seed}'
            with torch.no_grad():scores[name]=np.stack([model(v,t).cpu().numpy() for t in ts],axis=1)
            ...
            if mode=='rank_local_control':
                ...
                with torch.no_grad():scores[f'shuffled_image_seed{seed}']=np.stack([model(shuffled,t).cpu().numpy() for t in ts],axis=1)
    ...
    report=dict(scope='Entity phrase intervention feasibility pilot; no independent semantic labels or manual audit',intervention=MINUS_FIELD,image_counts={k:len(x) for k,x in ids.items()},pair_counts={k:len(x) for k,x in splits.items()},metrics=result,paired_delta_acc1_vs_inverse=paired,selection=history,seconds=time.time()-start)
