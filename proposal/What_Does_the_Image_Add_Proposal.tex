...
The next executable experiment holds deletion-trained checkpoints fixed and evaluates matched/natural captions without fitting. Report per-seed results, true prediction ensembles, tie-aware absolute and difference rankings, normalized changes and paired image-bootstrap intervals. Semantic labels remain human-review dependent. Strong direct region-description plus textual entailment/coverage baselines and an independent reader are required before publication expansion.
...
\textbf{Decision gates.} First establish label validity and frozen-model transfer. If simple baselines remain strongest, finish a rigorous course comparison and failure analysis. If labels cannot define a useful task, or no meaningful residual problem remains, stop the learned-method direction. A possible replacement is budgeted region acquisition: select the next region revealing the most new correct facts beyond an existing caption, using equal budgets and a fixed reader. This requires its own literature and feasibility checks, not an assumed novelty claim. At most two motivated variants (calibrated baseline residual and independently supervised coverage) may follow valid evidence; no unbounded tuning.
...
\textbf{Revised sequence.} P0: recover artifacts, provenance and final accounting. P1: audit 100 stratified semantic pairs. P2: run frozen deletion-trained checkpoints on matched/natural captions, then CPU evaluation. P3: decide whether to conclude the course comparison, try at most two motivated variants, or change direction. P4: only after a meaningful validation signal, confirm on untouched images and expand to independent regions/readers/datasets. Existing test images are exploratory after repeated inspection. Runnable commands are in \texttt{hpc/NEXT\_ROUND.md}; publication work is separately gated.
