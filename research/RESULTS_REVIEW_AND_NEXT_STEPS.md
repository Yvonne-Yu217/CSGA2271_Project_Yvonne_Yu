...
提前准备下一批有必要的任务。通过 batching、workers、prefetch、缓存复用和合理的 CPU/GPU 重叠保持推进。可让 subagent 并行审核数据、准备独立 baseline 或分析上一轮结果；主 agent 统一调度 GPU 和记账。只有测得有内存余量且提高有效吞吐时才同卡并发。不制造占用、不重复已完成计算；按照用户最新要求，不手动释放或取消 CPU/GPU，由服务器自行回收，避免会话中断。
